# ===============================
# YOUTUBE THUMBNAIL PANEL ANALYSIS SCRIPT (PPML VERSION) WITH SENTIMENT SCORE & MONTH FIXED EFFECTS
# ===============================
...
df <- df %>%
  mutate(
    ...
    time_month = format(published_at, "%Y-%m") # Add month fixed effect variable
  )
...
matched_df_face <- match.data(m.out.face) %>%
  mutate(time_month = format(published_at, "%Y-%m")) # Add month to matched set
...
# 7. PPML Regression with Channel and Month Fixed Effects and Robust SEs
# ----------------------------------------------------------------------
...
# Example: Plot predicted views over time for a specific treatment group
pred_time <- ggpredict(
